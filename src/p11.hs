...
-- Really slow solutions banged together, could use a lot of rework/study!

sol1 = foldr (\a b -> (if a == occupied then 1 else 0) + b) 0 (fixMoveAll initial)
sol2 = foldr (\a b -> (if a == occupied then 1 else 0) + b) 0 (queenFixMoveAll initial)

queenFixMoveAll m0 = let m = queenMoveAll m0 in if m /= m0 then queenFixMoveAll m else m
queenMoveAll from = Map.mapWithKey (\pos x -> queenMovePos from pos x) from
queenMovePos m pos x = queenMoveBlock (queenat m pos) x
queenMoveBlock block x = queenMoveN (length (filter (==occupied) block)) x
queenMoveN n x
    | n <= 0 && x == empty = occupied
    | n >= 5 && x == occupied = empty
    | otherwise = x
queenat m pos = concat 
    [ rayat m pos (-1,-1)
    , rayat m pos ( 0,-1)
    , rayat m pos ( 1,-1)
    , rayat m pos ( 1, 0)
    ...
    , rayat m pos (-1, 1)
    , rayat m pos (-1, 0) ]
rayat m (x,y) (dx,dy) = take 1 $ filter (\x -> x /= floor) $ fmap (at m) $ takeWhile indomain [(x+a*dx,y+a*dy) | a <- [1..]]

fixMoveAll m0 = let m = moveAll m0 in if m /= m0 then fixMoveAll m else m
moveAll from = Map.mapWithKey (\pos x -> movePos from pos x) from
movePos m pos x = moveBlock (blockat m pos) x
moveBlock block x = moveN (length (filter (==occupied) block)) x
moveN n x
    | n <= 0 && x == empty = occupied
    | n >=4 && x == occupied = empty
    | otherwise = x
blockat m (x,y) = fmap (at m) [(i,j) | i <- [x-1..x+1], j <- [y-1..y+1], (i,j) /= (x,y), indomain (i,j)]
at m xy = case Map.lookup xy m of { Just x -> x ; Nothing -> error ("oops " ++ show xy)}
initial = Map.fromList $ zip domain (concat input)
indomain (x,y) = x >= 0 && x < w && y >= 0 && y < h
domain = [(x,y) | y <- [0..h-1], x <- [0..w-1]]
w = length $ input!!0
h = length input
empty = 'L'
occupied = '#'
